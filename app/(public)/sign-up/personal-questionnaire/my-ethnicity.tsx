...
const MyEthnicity = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'ethnicity'],
        queryFn: () => getLookUpByName('ETHNICITY'),
    })

    const auth = useAtomValue(authAtom)

    const updateEthnicityMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_ETHNICITY',
                ethnicity: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-faith')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateEthnicityMu.isPending}
                    disabled={updateEthnicityMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
