...
const MyPrefAlcohol = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'alcohol'],
        queryFn: () => getLookUpByName('ALCOHOL'),
    })
    ...
    const updateAlcoholMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                ...
                upd_mode: 'USER_PREF_ALCOHOL',
                pref_alcohol: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-smoking')
        },
    })
