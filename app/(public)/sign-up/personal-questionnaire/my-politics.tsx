...
const MyPolitics = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'politics'],
        queryFn: () => getLookUpByName('POSPOLITICS'),
    })

    const auth = useAtomValue(authAtom)

    const updatePoliticsMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_POS_POLITICS',
                pos_politics: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-hobbies')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updatePoliticsMu.isPending}
                    disabled={updatePoliticsMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
