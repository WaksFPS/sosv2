...
const MyEyeColor = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'eyecolor'],
        queryFn: () => getLookUpByName('EYECOLOR'),
    })

    const auth = useAtomValue(authAtom)

    const updateEyeColor = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_EYE_COLOR',
                eye_color: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-kids-situation')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateEyeColor.isPending}
                    disabled={updateEyeColor.isPending || !selected}
                >
                    Next
                </Button>
            </View>
