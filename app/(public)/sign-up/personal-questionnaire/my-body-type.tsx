...
const MyBodyType = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'bodytype'],
        queryFn: () => getLookUpByName('BODYTYPE'),
    })

    const auth = useAtomValue(authAtom)

    const updateBodyTypeMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_BODY_TYPE',
                body_type: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-hair-color')
        },
    })

    const handleNext = () => {
        updateBodyTypeMu.mutate()
    }
    ...
    return (
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    onPress={handleNext}
                    isLoading={updateBodyTypeMu.isPending}
                    disabled={updateBodyTypeMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
