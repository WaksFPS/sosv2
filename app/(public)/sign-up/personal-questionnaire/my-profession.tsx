...
const MyHeight = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'profession'],
        queryFn: () => getLookUpByName('PROFESSION'),
    })

    const auth = useAtomValue(authAtom)

    const updateProfessionMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_PROFESSION',
                profession: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-pet-policy')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateProfessionMu.isPending}
                    disabled={updateProfessionMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
