...
const MyHairColor = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'haircolor'],
        queryFn: () => getLookUpByName('HAIRCOLOR'),
    })

    const auth = useAtomValue(authAtom)

    const updateHairColor = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_HAIR_COLOR',
                hair_color: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-eye-color')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateHairColor.isPending}
                    disabled={updateHairColor.isPending || !selected}
                >
                    Next
                </Button>
            </View>
