...
const MyExerReg = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'exercise-regiment'],
        queryFn: () => getLookUpByName('EXERREGIMEN'),
    })

    const auth = useAtomValue(authAtom)

    const updateExerciseRegimenMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_EXER_REGIMEN',
                exer_regimen: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-ethnicity')
        },
    })
    ...
    const handleSubmit = () => {
        updateExerciseRegimenMu.mutate()
    }
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateExerciseRegimenMu.isPending}
                    disabled={updateExerciseRegimenMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
