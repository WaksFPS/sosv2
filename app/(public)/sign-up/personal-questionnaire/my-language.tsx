...
const MyLanguage = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'language'],
        queryFn: () => getLookUpByName('LANGUAGE'),
    })

    const auth = useAtomValue(authAtom)

    const updateLanguageMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_LANGUAGE',
                language_code: selected.join(','),
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-profession')
        },
    })
    ...
    return (
        <SafeAreaView className="flex-1 mx-4 items-center justify-between space-y-4">
            <Stack.Screen
                options={{
                    title: 'Personal Details (12/16)',
                    headerStyle: {
            ...
            />
            <View className="flex-1 w-full items-center justify-center space-y-10">
                <View>
                    <TextScalable
                        fontSize={32}
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateLanguageMu.isPending}
                    disabled={updateLanguageMu.isPending || !selected.length}
                >
                    Next
                </Button>
            </View>
