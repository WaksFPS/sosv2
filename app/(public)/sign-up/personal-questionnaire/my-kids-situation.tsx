...
const MyKidsSituation = () => {
    const [selected, setSelected] = useState('')
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'kidsituation'],
        queryFn: () => getLookUpByName('KIDSITUATION'),
    })

    const auth = useAtomValue(authAtom)

    const updateKidsSituation = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_KID_SITUATION',
                kid_situation: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-education')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateKidsSituation.isPending}
                    disabled={updateKidsSituation.isPending || !selected}
                >
                    Next
                </Button>
            </View>
...
export default MyKidsSituation
