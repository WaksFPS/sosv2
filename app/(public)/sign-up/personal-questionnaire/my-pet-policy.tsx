...
const MyPetPolicy = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'petpolicy'],
        queryFn: () => getLookUpByName('PETPOLICY'),
    })

    const auth = useAtomValue(authAtom)

    const updatePetPolicyMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_PET_POLICY',
                pet_policy: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-politics')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updatePetPolicyMu.isPending}
                    disabled={updatePetPolicyMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
