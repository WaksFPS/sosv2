...
const MyFaith = () => {
    ...
    const lookUpRelationshipMu = useQuery({
        queryKey: ['lookup', 'religbelief'],
        queryFn: () => getLookUpByName('RELIGBELIEF'),
    })

    const auth = useAtomValue(authAtom)

    const updateFaithMu = useMutation({
        mutationFn: () =>
            updateUserFieldApi({
                user_id: auth.user.userId,
                who_updated: auth.user.userId,
                upd_mode: 'USER_RELIGION',
                relig_belief: selected,
            }),
        onSuccess: () => {
            router.push('sign-up/personal-questionnaire/my-language')
        },
    })
        ...
        <SafeAreaView className="flex-1 mx-4 items-center justify-between">
            ...
            <View className="w-full">
                <Button
                    ...
                    isLoading={updateFaithMu.isPending}
                    disabled={updateFaithMu.isPending || !selected}
                >
                    Next
                </Button>
            </View>
